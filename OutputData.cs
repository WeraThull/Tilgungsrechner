    ...
    public class OutputData
    {
        ...
        public void setOutput(OutputData[] value)
        {
            ...
            for (int i = 0; i < value.Length; i++)
            {
                ...
                form1.getDGV().Rows[i].Cells[1].Value = value[i].Kreditbetrag.ToString("#.00");
                form1.getDGV().Rows[i].Cells[2].Value = value[i].Zinsen.ToString("#.00");
                form1.getDGV().Rows[i].Cells[3].Value = value[i].Tilgung.ToString("#.00");
                form1.getDGV().Rows[i].Cells[4].Value = value[i].Annuitaet.ToString("#.00");
                form1.getDGV().Rows[i].Cells[5].Value = value[i].Restbetrag.ToString("#.00");
            }
        }
    }
}
